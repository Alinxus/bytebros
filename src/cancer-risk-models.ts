...
export interface LongitudinalScan {
  date: string;
...
export interface CancerRiskResult {
  ...
  trajectory: "decreasing" | "stable" | "increasing" | "unknown";
}
...
function calculatePatientBaseRisk(factors: PatientRiskFactors): { risk: number; contributions: RiskFactorContribution[] } {
  ...
  if (factors.geneticMarkers) {
    for (const marker of factors.geneticMarkers) {
      const mod = (RISK_MODIFIERS.geneticMarkers as Record<string, number>)[marker.toUpperCase()] || 1;
      risk *= mod;
...
function calculateXRayRisk(analysis: XRayAnalysisResult): { risk: number; findings: string[] } {
  let risk = 0.1;
...
function calculateLongitudinalRisk(scans: LongitudinalScan[]): {
  trajectory: "decreasing" | "stable" | "increasing" | "unknown";
  trendScore: number;
  ...
  for (const scan of sorted) {
    ...
    if (scan.analysis.overall_risk === "high") score += 0.3;
    else if (scan.analysis.overall_risk === "medium") score += 0.15;
    scores.push(score);
  }
  ...
  let trajectory: "decreasing" | "stable" | "increasing" | "unknown";
  if (progressionRate > 0.1) trajectory = "increasing";
  else if (progressionRate < -0.1) trajectory = "decreasing";
  else trajectory = "stable";

  return { trajectory, trendScore: progressionRate, progressionRate };
}

export function calculateCancerRisk(
  patientFactors: PatientRiskFactors,
  xrayAnalysis?: XRayAnalysisResult,
  longitudinalScans?: LongitudinalScan[]
): CancerRiskResult {
  ...
  let trajectory: "decreasing" | "stable" | "increasing" | "unknown" = "unknown";
  let progressionRate = 0;
  ...
  let riskLevel: CancerRiskResult["riskLevel"];
  if (lifetimeRisk >= 0.5) riskLevel = "very_high";
  else if (lifetimeRisk >= 0.3) riskLevel = "high";
  else if (lifetimeRisk >= 0.15) riskLevel = "medium";
  else if (lifetimeRisk >= 0.05) riskLevel = "low";
...
export function formatRiskReport(result: CancerRiskResult): string {
  return `
