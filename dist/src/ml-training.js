...
function trainRandomForest(data) {
    const options = {
        nEstimators: 100,
        maxFeatures: 0.8,
...
export async function trainModels() {
    ...
    const lrPredictions = lrModel.predict(normalizedData.features.map(f => [f])).map((x) => x[0]);
...
export function predictWithModels(features) {
    if (!cachedModels) {
