import { predictWithModels, initializeModels } from "./ml-training.js";
...
export async function predictCancer(features, riskFactors, longitudinalData) {
    await initializeModels();
    const featuresArray = featuresToArray(features);
    const mlResult = predictWithModels(featuresArray);
    ...
    if (riskFactors) {
        ...
        if (riskFactors.geneticMarkers) {
            for (const marker of riskFactors.geneticMarkers) {
                const geneRisk = CANCER_RISK_FACTORS.geneticMarkers[marker.toUpperCase()];
                if (geneRisk) {
    ...
    const riskLevel = finalRiskScore >= 60 ? "very_high"
        : finalRiskScore >= 40 ? "high"
            : finalRiskScore >= 20 ? "medium"
                : "low";
...
export function predictFromXRay(input) {
    ...
    const currentDiagnosis = {
        prediction: diagnosisScore > 50 ? "malignant" : "benign",
