...
const applyQualityPenalty = (confidence, quality) => {
...
cancer.post("/predict", zValidator("json", predictionSchema), async (c) => {
    const body = c.req.valid("json");
    let diagnosisResult = null;
    let xrayResult = null;
    if (body.features && Object.keys(body.features).length > 0) {
        diagnosisResult = await predictCancer(body.features, body.riskFactors, body.longitudinalData);
    }
    if (body.imageUrl || body.imageBase64) {
        try {
            xrayResult = await analyzeXRay(body.imageUrl || Buffer.from(body.imageBase64, "base64"));
        }
        catch (err) {
...
cancer.post("/risk-assessment", zValidator("json", z.object({
...
cancer.post("/longitudinal", zValidator("json", z.object({
    ...
    const body = c.req.valid("json");
    const result = await predictCancer(body.currentFeatures, {}, { previousPredictions: body.previousScans?.map(s => ({ date: s.date, riskScore: s.result === "malignant" ? 80 : 30 })) });
    return c.json({
        timestamp: new Date().toISOString(),
...
cancer.post("/xray", zValidator("json", z.object({
    imageUrl: z.string().url().optional(),
    imageBase64: z.string().optional(),
    ...
    const result = await analyzeXRay(body.imageUrl || Buffer.from(body.imageBase64, "base64"));
...
cancer.post("/xray-history", zValidator("json", z.object({
    ...
    if (body.currentImageUrl || body.currentImageBase64) {
        try {
            currentAnalysis = await analyzeXRay(body.currentImageUrl || Buffer.from(body.currentImageBase64, "base64"));
        }
        catch (err) {
            console.error("[X-ray] Error:", err);
        }
    ...
    if (body.previousImages) {
        for (const img of body.previousImages) {
            if (img.result) {
                ...
                try {
                    const analysis = await analyzeXRay(img.imageUrl || Buffer.from(img.imageBase64, "base64"));
                    previousAnalyses.push({
                        date: img.date,
...
cancer.post("/consensus", zValidator("json", z.object({
    xray: z.object({
    ...
    const inputs = [];
    const addInput = (label, score, weight) => {
...
cancer.post("/risk-predict", zValidator("json", z.object({
    patient: z.object({
    ...
    const body = c.req.valid("json");
    const result = calculateCancerRisk(body.patient, body.xrayAnalysis, body.longitudinalScans);
    return c.json({
        timestamp: new Date().toISOString(),
...
cancer.post("/breast-cancer-predict", zValidator("json", z.object({
...
cancer.post("/mammography", zValidator("json", z.object({
    imageBase64: z.string().optional(),
    imageUrl: z.string().url().optional(),
    patientData: z.object({
        age: z.number().optional(),
...
cancer.post("/analyze", zValidator("json", z.object({
    imageBase64: z.string().optional(),
    imageUrl: z.string().url().optional(),
    ...
    try {
        ...
        if (body.patientData) {
            const riskFactors = [];
            if (body.patientData.age && body.patientData.age > 50)
                riskFactors.push("Age > 50");
            if (body.patientData.familyHistory)
...
cancer.post("/longitudinal-track", zValidator("json", z.object({
    currentImageBase64: z.string().optional(),
    currentImageUrl: z.string().url().optional(),
    previousScans: z.array(z.object({
        date: z.string(),
...
cancer.post("/triage", zValidator("json", z.object({
    symptoms: z.array(z.string()).optional(),
    patientData: z.object({
        age: z.number().min(18),
        gender: z.enum(["male", "female"]),
...
cancer.post("/second-opinion", zValidator("json", z.object({
