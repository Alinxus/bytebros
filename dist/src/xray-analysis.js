...
export async function analyzeXRay(imageData, patientSymptoms, patientHistory) {
    let base64Data;
    try {
    ...
    if (mlHealthy) {
        ...
        if (mlResult.success && mlResult.analysis) {
            ...
            const findings = analysis.findings.map(f => ({
                ...
                location: "chest",
                severity: f.risk_level === "high" ? "severe" :
                    f.risk_level === "medium" ? "moderate" : "mild",
            ...
            return {
                ...
                cancerType: analysis.findings.some(f => ["lung lesion", "mass", "nodule"].includes(f.pathology.toLowerCase())) ? "lung" : undefined
            };
        }
    }
...
async function analyzeWithGPT(base64Data) {
    if (!openai) {
        return {
            hasAbnormality: false,
            findings: [],
